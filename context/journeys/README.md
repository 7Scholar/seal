...
**Nine scenarios run**, all green: `first-run` (8 checks), `return-and-use` (9), `interrupted-rekey` (5), `living-with-it` (6), `plaintext-expiry` (6), `settling-in` (8), `deploy-script` (7), `bad-day` (9) and `freshness` (5). Two further specs are **not journeys** and do not appear above: `window-frame` (4), which drives the window *as a window*, and `manage-filter` (5), which drives the manage surface's filter against a file no journey's fixture buries deeply enough to need it. Both exist because their defect classes are invisible to a journey that asks whether a *task* completes — every journey passed while three surfaces had no title bar at all.
