...
**Five scenarios now run:** `first-run` (8 steps), `return-and-use` (9), `interrupted-rekey` (5), `living-with-it` (6) and `plaintext-expiry` (6).
...
[protect-a-repo.md](protect-a-repo.md) is driven through step 6 of 7, inside the same runs; its remaining step is undriven because no scenario stages it.

[use-a-secret.md](use-a-secret.md) is driven at steps 1–5 and **now step 8**, leaving 6 and 7. Neither is blocked — step 7 drives the **CLI binary** rather than the desktop app, so it needs a different harness shape. Driving step 8 opened one finding, shared with `living-with-it` and described below.
