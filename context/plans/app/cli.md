...
**The search past the sibling can be switched off.** Setting `SEAL_IGNORE_INSTALLED_APP` stops the search after the sibling check, so nothing installed on the machine is consulted. This exists because the platform's own lookup finds an application registered anywhere, which no environment a test controls can hide: without the switch, the test asserting the not-found path passes only on a machine where Seal happens not to be installed, and starts failing the moment a developer installs it. The switch makes that test hermetic rather than accidentally passing.
