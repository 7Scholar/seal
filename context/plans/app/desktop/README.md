...
**`ui/navigation/editing/` is framed with its forks settled, and it is the first concern since `dotenv.md` to reach the whole depth of this node at once.** The file altitude reads an environment-variables file and changes the value of a variable that already exists; it cannot create, delete, rename, reorder or duplicate one, and the owner's stated end state is managing those files in Seal instead of writing them by hand. It touches four layers together — the surface, the editor's rules, `commands.md`'s `save`, whose `(key, value)` signature can express no structural change, and `dotenv.md`'s line model, which is built for changing a value in place rather than mutating the sequence of lines.

The fork that decided the rest went to a **structural edit list** rather than whole-document replacement, on the grounds that `dotenv.md`'s byte-exactness holds *by construction* — a line no operation names is never re-rendered — and the alternative converts that guarantee into reconciliation logic that must be kept right on every edit, failing quietly inside a sealed file the user cannot inspect. `dotenv.md` therefore grows structural mutation over stable row identity while its round-trip guarantee stays exactly as it is; the edit list is the vocabulary `commands.md` carries.
