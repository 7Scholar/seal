...
**`ui/navigation/editing/` is newly framed and blocked on the product owner**, and it is the first concern since `dotenv.md` to reach the whole depth of this node at once. The file altitude reads an environment-variables file and changes the value of a variable that already exists; it cannot create, delete, rename, reorder or duplicate one, and the owner's stated end state is managing those files in Seal instead of writing them by hand. Answering it touches four layers together — the surface, the editor's rules, `commands.md`'s `save`, whose `(key, value)` signature can express no structural change, and `dotenv.md`'s line model, which is built for changing a value in place rather than mutating the sequence of lines. The fork that decides the rest is whether an edit crosses the boundary as a structural edit list or as a whole-document replacement, because `dotenv.md`'s byte-exactness currently holds *by construction* — an untouched line is emitted verbatim because nothing re-rendered it — and one of those two directions turns it into reconciliation logic that must instead be kept right.
