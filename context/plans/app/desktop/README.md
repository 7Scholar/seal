...
**[living-with-it](../../../journeys/living-with-it.md) was driven for the first time**, and produced one fix here plus two framed concerns. The fix is `commands.md`'s: **the overview served the registry's recorded state rather than what reconciliation observed**, so a managed file deleted outside Seal read as `Sealed` with a live open control. The registry library was correct throughout and its suite proves it — the defect was entirely in the consumer, which computed the reconciliation, took only the exposure flag from it, and then read `last_known` anyway. Every Rust and interface suite passed while the driven application showed the wrong state, which is this axis's signature shape. Fixed, non-vacuity confirmed, and now asserted in the real application by a fourth scenario.

What that journey found and did **not** fix is framed as `ui/navigation/freshness.md` and **blocked on the product owner**: the interface re-reads disk only when the session unlocks, and nothing anywhere states that everything *is* protected. Its step 5 (plaintext expiry) is still undriven, and the obstacle is a hard-coded fifteen-minute lifetime rather than a missing scenario — a lifetime seam of the folder-pick's shape would belong to `commands.md`, and is recorded on `journey-harness.md` rather than built.

**`ui/navigation/editing/` is built and driven**, and it is the first concern since `dotenv.md` to reach the whole depth of this node at once. The file altitude read an environment-variables file and changed the value of a variable that already existed; it now carries create, rename, delete, duplicate, reorder, enable-or-disable and the correction of a malformed line, through four layers together — the surface, the editor's rules, `commands.md`'s `save`, and `dotenv.md`'s line model.

The fork that decided the rest went to a **structural edit list** rather than whole-document replacement, on the grounds that `dotenv.md`'s byte-exactness holds *by construction* — a line no operation names is never re-rendered — and the alternative converts that guarantee into reconciliation logic that must be kept right on every edit, failing quietly inside a sealed file. `dotenv.md` grew structural mutation over stable row identity while **its fourteen round-trip tests passed unchanged**, which is the evidence that the guarantee survived.

Two consequences reached this node's own surface. `commands.md`'s `reveal` now addresses a **row** rather than a key, which closed a live defect: a file defining a key twice returned the first row's value when the second was revealed. And `save` carries an edit list applied **all-or-nothing** and returns the reparsed view, because a partially-applied edit on a sealed file is the failure with no recovery.
