...
**Saving sends back an edit list, never the file.** The interface sends an ordered list of structural operations — set a value, rename a key, enable or disable, insert, remove, replace a malformed line, reorder — and the plaintext held in Rust is edited through the lossless env model and re-sealed from memory. The whole secret never makes the round trip. The list is applied **all-or-nothing**: an operation naming a row the file no longer holds refuses the entire list without writing, since a partially-applied edit on a sealed file is the failure with no recovery. [ui/navigation/editing/boundary.md](ui/navigation/editing/boundary.md) owns the vocabulary.

**A save returns the reparsed view.** Insertion and removal change which rows exist, so an interface holding the pre-save view would address rows that no longer do; returning the fresh view makes the post-save state authoritative in one round trip.
...
- [x] Enforce the boundary rule in the shape of the commands: masked structure on open, per-row reveal as raw bytes, an edit list rather than files on save.
