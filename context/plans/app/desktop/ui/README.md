...
- [~] navigation/ -> the navigation model: breadcrumb routing over three full-width altitudes, the themes, and the title bar as a real window surface. **Audited and part-rebuilt: the repositories grid is finished to production depth with every state, and the interface now has an icon system. The files list, the file surface and the root segment's missing switcher remain.**
