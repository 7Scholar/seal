...
**`navigation/editing/` is where this group's work now stands: framed, settled, and unbuilt.** The product owner asked for the file altitude to become a full way of working with an environment-variables file, against the end state of no longer writing those files by hand. What exists is reading a file and changing the value of a variable that already exists — there is no create, delete, rename, reorder or duplicate anywhere in the stack, and `save`'s `(key, value)` signature can express none of them. It is a folder because the answer reaches the surface, the editor's rules, the command boundary and `dotenv.md`'s line model together.

Its forks are answered and its Approach committed, with six children carved. The decision that shapes this group's work is that an edit crosses as a **structural edit list over stable row ids**: identity is neither the key — so a rename preserves a row rather than routing through a destructive verb — nor the line number, which is not stable across the very operations being added. Two of the owner's other decisions change what the editor *is* rather than what it can do: a commented-out assignment becomes a **disabled variable** with an enable/disable control, and a malformed line becomes an **editable row** with a Correct that refuses rather than guesses — so the two notices this surface currently shows become rows the user can act on.
