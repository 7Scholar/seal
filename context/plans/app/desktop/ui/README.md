...
It is **in flight, not complete.** The model works and is driven end to end, but three of its surfaces were built only for the state where everything is present, and the breadcrumb diverges from the reference the owner supplied — a review of the running application found it reads as amateur, and no test, journey or drive had caught any of it. Its cursor holds the specifics; [SURFACE_AUDIT.md](../../../../../docs/plans/SURFACE_AUDIT.md) is the pass that now exists to catch this class, and the next session here runs it.

Two further concerns have since been taken in from the product owner and framed under it, both `TBD` and research-first: the **manage surface** carried to the depth the grid now has — it was never in the audit, and a folder holding no candidates is inert on click — and the **palette**, which was never chosen, only tokenised in place from the values the first screens happened to carry. `navigation/`'s cursor holds both.
