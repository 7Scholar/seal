...
- [~] navigation/ -> the navigation model: breadcrumb routing over three full-width altitudes, the themes, and the title bar as a real window surface. **Its depth pass is complete across all three altitudes, and `editing/` has given the file altitude its full vocabulary — built and driven, bar bulk entry.**
...
**`navigation/editing/` gave the file altitude its full vocabulary, and it is built and driven.** The product owner asked for the surface to become a complete way of working with an environment-variables file, against the end state of no longer writing those files by hand. A user can now create, rename, delete, duplicate, reorder and enable-or-disable a variable, and correct a malformed line, carrying all of it through one save. What is deliberately not built is bulk paste, framed as that node's last child.

Two of its decisions change what the editor **is** rather than what it can do, and both come from the owner. A commented-out assignment is a **disabled variable** with an enable/disable control, so the two notices this surface used to carry above its rows are now rows the user acts on; and a malformed line is an **editable row** whose `Correct` refuses rather than guesses, retiring the count that stood in for it.

Its findings are recorded in that node. The one worth repeating here is that `reveal` addressed values by key, so a file defining a key twice — the case this group's own notice warns about — handed the second row the first row's secret. Addressing by row id closed it.
