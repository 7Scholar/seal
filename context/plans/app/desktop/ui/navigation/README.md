...
- [~] editing/ -> the full editing vocabulary for an env file: create, rename, delete, duplicate, reorder, and commented-out variables as a disabled state. **Its forks are settled and its Approach committed; six children are carved and none is built.**
...
**[editing/](editing/README.md) is where this node's work now stands: framed, its forks settled by the product owner, and nothing built.** The file altitude can read a file and change the value of a variable that already exists; it cannot create, delete, rename, reorder or duplicate one, and the owner's stated end state is that they stop writing environment variables by hand altogether. It is a folder rather than a plan `.md` because it reaches through four layers at once — the surface, the editor's rules, the `save` command whose `(key, value)` signature can express none of it, and [dotenv.md](../../dotenv.md)'s line model, which is built for changing a value in place rather than for mutating the sequence of lines.

The load-bearing fork went to a **structural edit list over stable row ids**, which is what keeps byte-exactness a guarantee that holds by construction — a line no operation names is never re-rendered — rather than reconciliation logic that must be kept right on every edit inside a file the user cannot inspect. The owner's other decisions shaped the surface as much as the model: `# FOO=bar` is a **disabled variable** with an enable/disable control rather than a comment, under a strict rule that deliberately fails towards prose; ordinary comments and blank lines are **ignored** and hold their positions, so a reorder moves variables out from under a heading and the interface cannot say so — a fidelity-for-determinism trade taken knowingly; malformed lines become **editable rows** with a Correct that refuses rather than guesses; and ceremony sits at the **save**, which inspects its batch and confirms once when the batch removes anything, rather than in front of each delete.
