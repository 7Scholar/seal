...
- [!] editing/ -> the full editing vocabulary for an env file: create, delete, rename, reorder and what else the owner's "stop writing env files by hand" end state requires. **Blocked — awaiting answers in [editing/QUESTIONS.md](editing/QUESTIONS.md).**
...
**[editing/](editing/README.md) is newly framed and blocked**, taken in from the product owner. The file altitude can read a file and change the value of a variable that already exists; it cannot create, delete, rename, reorder or duplicate one, and the owner's stated end state is that they stop writing environment variables by hand altogether. That is a higher bar than adding the missing verbs, and it is a folder rather than a plan `.md` because it reaches through four layers at once — the surface, the editor's rules, the `save` command whose `(key, value)` signature can express none of it, and [dotenv.md](../../dotenv.md)'s line model, which is built for changing a value in place rather than for mutating the sequence of lines.

Its forks are raised and unanswered. The load-bearing one is whether an edit crosses the boundary as a **structural edit list** or as a **whole-document replacement**, because that decides whether byte-exactness stays a guarantee that holds by construction — an untouched line is emitted verbatim because nothing re-rendered it — or becomes reconciliation logic that has to be right. The rest follow from it: what happens to the comments and blank lines the editor does not draw, whether structural verbs stay pending under Save or commit with an undo, and how much ceremony stands in front of deleting a secret that exists nowhere else.
