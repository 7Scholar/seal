...
- [x] file-jump.md -> the breadcrumb menu as a two-level surface, so a file in any repository is reachable without landing on its repository first. Built and driven; the three flat switchers and the component behind them are gone.
...
**[editing/](editing/README.md) is built and driven, bar one child.** The file altitude could read a file and change the value of a variable that already existed; it can now create, rename, delete, duplicate, reorder and enable-or-disable one, and correct a malformed line, with all of it carried through a single save. Nine driven checks in the real webview (`bun run e2e:editing`) confirm the vocabulary where unit tests cannot see.

The load-bearing fork went to a **structural edit list over stable row ids**, which keeps [dotenv.md](../../dotenv.md)'s byte-exactness a guarantee that holds by construction — a line no operation names is never re-rendered — rather than reconciliation logic that must be kept right inside a file the user cannot inspect. Its fourteen round-trip tests **pass unchanged** across the growth, which is the evidence that held.

The owner's other decisions shaped the surface as much as the model: `# FOO=bar` is a **disabled variable** with an enable/disable control, under a strict rule that deliberately fails towards prose because enabling a misread comment would write a live variable that was never meant to exist; ordinary comments and blank lines are **ignored** and hold their positions, so a reorder moves variables out from under a heading and the interface cannot say so — a fidelity-for-determinism trade taken knowingly; malformed lines are **editable rows** whose `Correct` refuses rather than guesses; and ceremony sits at the **save**, which inspects its batch and confirms once when the batch removes anything.

**The row was then carried through a density pass**, because the first build put all eight verbs side by side in it. [editing/row-density.md](editing/row-density.md) collapses them to three plus the `Overflow` menu this node's own tiles already use, and makes the row one tab stop rather than seven — which at four hundred variables is the difference between 2,800 and 400. It is the same lesson this node keeps relearning: a row was **192px tall, four lines**, and every unit test passed throughout.
...
**All five are now built and driven, and the trail's flat switcher is gone.** [file-jump.md](file-jump.md) was the last of them, and it turned out to be the smallest of the five rather than the largest: the scope had framed it around fetching and holding every repository's file list, and the application already holds exactly that — one observation carrying every repository with its files, re-read on focus and on the sweep. So the menu draws the snapshot the screens draw, and is as fresh as they are.

The three flat switchers collapse into **one menu on the trail's last segment**, which settles [breadcrumbs.md](breadcrumbs.md)'s standing open thread — the file switcher reaches across every repository, by reaching *through* the repository panel rather than by flattening two lists into one. It also carries the state language into the chrome: a developer standing anywhere can see what is sealed in every repository without navigating. That does **not** reopen [living-with-it](../../../../../journeys/living-with-it.md)'s finding 3, which asked for the healthy answer in a second *without reading*; this needs a gesture and a read. It is recorded here because the mechanism that finding said did not exist has moved closer.

**Two defects were found by looking at the running application rather than by any test.** The title bar's `button` rule out-specifies the menu's own, so every row in both panels rendered at 27px instead of 38 — caught by the target-size guard the row work had already put in `manage-readable`, which is the one check that measures rather than queries. And nesting the second panel inside a scrolling first panel clipped it away entirely: the running window showed the file list with **no repository list behind it at all**, while every interface test passed, because both panels were in the document. The guard now measures every row in both panels, and fails by name when the scoping is removed.

**One defect was found by driving it.** Activation was left to the native `<button>` behaviour under `role="menuitem"`, and the harness's synthesized `Enter` reached the row without activating it — the menu opened, walked and dismissed correctly from the keyboard while being impossible to use from it. The interface tests could not see it, because the testing library activates a button by clicking. [MEMORY.md](../MEMORY.md) records why the explicit handler must not be deleted as redundant.
...
**Nothing of the redesign is left.** The five behavioural nodes, the ordering they imposed, and every cosmetic surface — the repository tree, the file altitude's variable line, the add-files surface, the password change, the dialogs and the trail's menu — are built, driven and stamped.

Three contrast pairs in the specified palette missed their WCAG floor. They were measured, put to the owner, and darkened on the owner's instruction; [palette.md](palette.md) records the audit and the one counter-intuitive result — `--faint` is no longer the faintest value in the palette, because the washes now set that floor.
