...
- **Name column misaligns by 1px between kinds.** A directory's name starts at `x = 149.95`, a file's at `x = 148.95` — because `.tree__name` is monospace but `.tree__row[data-kind="directory"] .tree__name` reverts to `inherit` ([styles.css:438-439](../../../../../../../ui/styles.css#L438-L439)), and the two fonts have different left side bearings. Every directory row in the tree sits one pixel right of every file row.
