...
- **The selection model on this surface is due to be retired, and its replacement is not built.** The redesign draws this altitude as a tree with no checkbox on any row, moving multi-file sealing to a single *Seal every file* in the repository's menu — [seal-all.md](seal-all.md), which is framed and itself waiting on [ceremony.md](ceremony.md). Until that chain is worked, this surface keeps its checkboxes and its batch bar: removing them first would take away the only way to seal several files at once, and with it the route [the bad-day scenario](../../../../../journeys/living-with-it.md) drives for the recency warning.
...
- Nothing virtualizes, and there is no excessive-state treatment. The count makes the size visible; the DOM cost is untouched, as on the repositories list.
