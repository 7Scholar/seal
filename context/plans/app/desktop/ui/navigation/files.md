...
- **The selection model on this surface is due to be retired.** The redesign draws this altitude as a tree with no checkbox on any row; its replacement, *Seal every file*, is now built and driven ([seal-all.md](seal-all.md)), so the checkboxes and the batch bar can go when the tree is built.
