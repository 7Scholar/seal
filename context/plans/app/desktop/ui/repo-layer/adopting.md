...
- Whether a filter or search over the tree is needed. **Settled: it was, and it is built** — [manage-surface.md](../navigation/manage-surface.md) owns it, with the two requirements this thread set (restore the prior expansion on clear, never touch selection) holding as stated and guarded. The deciding argument was structural rather than a matter of feel: expansion follows the detected files, so an undetected file is reachable only by hand-opening its chain, which is precisely the case undetected files are selectable for.
