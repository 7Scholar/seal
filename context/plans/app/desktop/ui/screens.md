...
The editor opens a managed file **whatever its on-disk state**, a readable one exactly as a sealed one. Saving always leaves the file sealed, since the engine writes no plaintext to a managed path — so on a file that is currently readable the save control says **Save and seal** rather than *Save*. The control names the consequence because sealing is the one thing about the save a user cannot undo, and a control that quietly encrypts a file the user was editing in the clear is the surprise this vocabulary exists to prevent.
