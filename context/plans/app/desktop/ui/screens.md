...
**The secret value control** holds the reveal contract: the masked text, a toggle whose state lives in `aria-pressed` rather than in its label or its glyph, an accessible name that names its variable so rows are distinguishable, and a live announcement of the state. Every one of those is a separate assertion, because a surveyed product shipped this control with state conveyed by icon alone — twice.

**The value itself is now that control** ([navigation/editing/value-as-button.md](navigation/editing/value-as-button.md)): one press reveals, a second edits, and **Escape** puts a revealed value back. There is no copy control — the design removed it, and this tree had already recorded that its clipboard had no timer, which is a hole rather than a feature. A revealed value is selectable text instead, so the operating system's own copy still reaches it and no untimed clipboard write is ours.
...
**The four conditions a managed file can be in are named in one place**, `ui/state.ts`, and every surface asks it rather than deciding for itself: the seal broke, sealed, not sealed, gone from disk. The module exists because the distinction above — `alert` against `state` — is the one this interface has already got wrong once, and a rule stated in prose but re-derived at four call sites is a rule with four chances to drift. Its consumers are the repositories list's ticks, the file tree, the breadcrumb menu and the editor's header, and a fifth condition — **unknown** — is reserved for a re-read that failed ([navigation/freshness.md](navigation/freshness.md) owns when it applies).
...
**The unlock gate** is a shield of coal-fine sand drawn over a lit ground: the pointer parts the grains and they close again at once, and every typed character answers with a brief glow somewhere in the shield. The password is typed straight at the screen — the field is real but visually hidden, which keeps the label, the masking, and the autocomplete opt-outs — and Enter is the single verb: it attempts the unlock, a working state shows while the key is derived, and a wrong password says the files were not opened, clearing the attempt so retyping and Enter always submit fresh. The shield honours the reduced-motion preference and renders nothing at all where no drawing context exists, which is also what keeps the screen testable.

The overlay above the sand is the **seal mark**, the wordmark **SEAL**, a hint, a row of **pressed dots**, and the status. Four things follow from that shape:

- **The screen does not name its own state.** The wordmark is the product, not the condition; *Seal is locked* and *Choose your master password* are both gone as headings, because the hint below already says which of the two the user is in — *type your master password* against *choose a master password* — and a heading repeating it said the same thing twice.
- **A dot is pressed into the sand for every character typed**, the most recent one darker than the rest. The input is deliberately invisible, and until this existed nothing at all confirmed that a keystroke had landed. It is capped at twenty; past that the count stops growing rather than the row overflowing, which costs nothing because the confirmation a user needs is that *something moved on this keystroke*.
- **The mark turns to the sand's alarm colour when a password is refused**, and its inner stroke to the sand's own ground so the silhouette survives the change. It is the only element on the screen that changes colour, and it changes for one reason.
- **Two messages are shorter than they were.** The refusal drops *nothing was changed* and *type it again*; the auto-lock notice drops *everything stayed sealed*. Both facts are true and neither was earning its place at the moment it appeared. The mismatch and the failed-establishment messages keep *nothing was set* and *nothing was changed*, because those are the two moments where a user could reasonably fear they had just destroyed something. The first-run irreversibility warning keeps its full weight and is stated as the standing status rather than folded into the hint.
