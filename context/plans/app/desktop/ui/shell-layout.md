...
The safety properties the per-file path already has are not weakened by the batch. The irreversibility acknowledgement gate is unchanged — it is per-registry and fires once, before the first seal of any kind. Neither route consults recency any more, so the two cannot diverge on it; [navigation/ceremony.md](navigation/ceremony.md) owns that removal and the report standing where the warning stood. The batch is now reached without a selection at all ([navigation/seal-all.md](navigation/seal-all.md)), which is why the set it acts on is a property of the repository rather than of what the user ticked.
