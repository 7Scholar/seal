...
**Writing edited contents back is two operations, separated by their guard rather than by their body.** Both take plaintext held in memory and replace the path with a sealed file through the same lock and atomic replacement. They differ only in what they require of the file they are replacing: the **re-seal** form refuses a file that is not already sealed, which is what stops a save from overwriting a file the caller never opened as sealed; the **seal-from-memory** form has no such requirement, and is what lets an editor save a managed file that was still readable on disk. Neither writes plaintext to the path — the file is sealed in both directions — so the rule that a managed file's on-disk state never moves back to plaintext is untouched. Splitting the guard rather than the operation is deliberate: a second code path writing the same bytes through a different replacement would be the place the two silently diverge.
