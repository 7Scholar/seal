...
## 2026-09-06 — The check job's two prerequisites look unrelated to checking, and are not

Before it lints, the Rust job builds the command-line tool into `src-tauri/binaries/` and, on Linux, installs `expect` alongside the desktop libraries. **Why:** the desktop crate declares the tool as a bundled sidecar, and `tauri-build` refuses to build when a declared resource is missing — so without that step a fresh checkout cannot be linted or tested at all, and the failure names a bundle path in a job that bundles nothing. `expect` is what drives the password prompt through a real terminal; macOS ships it and Linux does not, so on the runner it would otherwise skip. **Mistake it prevents:** deleting the sidecar build as a packaging step that wandered into the lint job, or dropping `expect` from a list that otherwise holds only desktop libraries — the first turns every check red with a message pointing at packaging, the second turns the terminal proof into a silent no-op on Linux while the job still reports green.
