...
Being a plain binary rather than a bundle grants no exemption: an unsigned command-line tool that arrives quarantined is killed behind the same malware dialog, with no override button where a bundle would at least offer right-click-to-open. Only the delivery mechanism saves it, which is why continuous integration asserts on every change that a curl-delivered tarball extracts to an ad-hoc-signed binary carrying no quarantine.
