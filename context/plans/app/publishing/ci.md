...
**The Rust job** runs on macOS and Linux, because the application targets both and the filesystem behaviour the engine depends on differs between them. On Linux it installs the desktop system libraries, since the Tauri crate does not build without them, and `expect`, which macOS already carries and the terminal-driven test cannot run without. It then builds the command-line tool into `src-tauri/binaries/` before linting: the desktop crate declares that tool as a bundled sidecar, and `tauri-build` fails outright when the declared resource is absent, so a fresh checkout cannot be linted or tested at all until it exists. It then builds the interface, for the same class of reason: the desktop crate embeds `dist/` at compile time, so `generate_context!` panics when that directory is absent and no cargo command produces it — the interface is a build input of the Rust crate, not a parallel concern. Formatting runs ahead of both builds, so a formatting failure still reports in seconds. Finally it sets the variable that turns a skipped terminal-driven test into a failure, because that test proves the password prompt reaches a real terminal and a silent skip would verify nothing.
...
Two rules cut across all of them. Skips are permitted so a fresh clone is green without installing anything, but every skip that guards a real claim has an environment variable that converts it into a failure, and continuous integration sets it. That covers **every** reason a guarded test can decline to run, not only the obvious one: the terminal-driven test skips both when no pseudo-terminal can be allocated and when `expect` is missing, and the variable makes each of those a failure — a guard that honours the variable on one path and skips silently on the other reports green while proving nothing. And the toolchain is pinned rather than floating, so an upstream change surfaces as a deliberate update rather than a mysterious failure.
