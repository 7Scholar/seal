...
**Landing your work (git). This repository has no remote, so you work on `main` and commit there — do not create a branch.** The rule and its reasoning are owned by [GIT_WORKFLOW.md](GIT_WORKFLOW.md) under **While this repository has no remote**; what follows is what it means for you in a session.

A branch exists to become a pull request. With no origin, there is no push, no PR, no review and no merge — so a branch is a local pointer that only hides your work from the next session, which reads `main`. **Ignore every instruction in this tree telling you to branch off `main`, hand a branch off, or wait for the user to land one**; they describe the workflow that resumes if an origin is ever added, and they are superseded until then.

1. **Stay on `main` and commit freely.** Don't branch, and don't branch "to be safe" — unmerged work is invisible work. Confirm with `git branch --show-current` if unsure.
2. **Run one task at a time.** The subtree partition below relied on separate branches conflicting loudly at merge time on a shared `coverage.json`. On `main` there is no merge, so nothing catches two overlapping tasks — they simply interleave.
3. **Keep to a plan subtree no other in-flight task is touching.** If two tasks ever overlap on a `coverage.json`, tell the user; don't hand-edit it.
4. **Stamp coverage as your final commit** — commit the code and prose first (**Ending a session** step 5), then run `add_to_coverage` (step 6) once everything is settled, and don't edit those files again afterward. This matters exactly as much without a branch: it is what keeps the detector clean.
5. **Stop at the commit and report.** Still **never `git push`** and never run `gh pr create` — there is nowhere to push to, and adding a remote is the user's decision, not a step you take to satisfy these instructions.
