...
**Landing your work (git). You work one plan on a branch off `main` and stop at the commit; the user pushes, opens the pull request, reviews and merges.** The rule and its reasoning are owned by [GIT_WORKFLOW.md](GIT_WORKFLOW.md) under **The shape**; what follows is what it means for you in a session.

1. **Branch off `main` and commit freely.** Name the branch for the work it carries. Confirm with `git branch --show-current` if unsure.
2. **Keep to a plan subtree no other in-flight task is touching.** Disjoint subtrees touch disjoint leaf folders, so their coverage never overlaps; a `coverage.json` conflict on a pull request is the alarm that two tasks were given overlapping territory. Tell the user; don't hand-edit it.
3. **Stamp coverage as your final commit** — commit the code and prose first (**Ending a session** step 5), then run `add_to_coverage` (step 6) once everything is settled, and don't edit those files again afterward. Stamping last is what makes the branch land free of drift.
4. **Stop at the commit and report.** **Never `git push`** and never run `gh pr create`: the whole remote side is the user's, and the branch you hand over is already complete — code, prose and coverage reconciled.
5. **The merge must be a merge commit**, never squash or rebase, or every stamp on the branch points at a rewritten SHA and the change reads as drift on `main`. That is the user's to enforce as a repository setting; raise it if you see a branch landed any other way.
