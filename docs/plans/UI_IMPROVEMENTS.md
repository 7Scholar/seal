...
- **One session is one task.** The landing rules in [INSTRUCTIONS.md](INSTRUCTIONS.md) apply unchanged: a branch off `main`, coverage stamped last, and no push. Because the session spans many plans' coverage, it breaks the one-subtree partition ordinary tasks keep ([GIT_WORKFLOW.md](GIT_WORKFLOW.md)) — so a UI session must not run concurrently with another in-flight task that touches frontend-covered files. The collision would surface as a `coverage.json` conflict at merge, which is late and confusing; running them one at a time is what actually prevents it.
...
7. **Stop at the commit on your branch and report** ([INSTRUCTIONS.md](INSTRUCTIONS.md)). Never push; the user lands it.
