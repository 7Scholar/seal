...
- **One session is one task.** The landing rules in [INSTRUCTIONS.md](INSTRUCTIONS.md) apply unchanged: while the repository has no remote that means committing on `main`, stamping coverage last, and never pushing. Because the session spans many plans' coverage, it breaks the one-subtree partition ordinary tasks keep ([GIT_WORKFLOW.md](GIT_WORKFLOW.md)) — so a UI session must not run concurrently with another in-flight task that touches frontend-covered files. Without branches nothing will flag the collision, which makes running them one at a time the whole of the protection rather than a first line of it.
...
7. **Stop at the commit on `main` and report** ([INSTRUCTIONS.md](INSTRUCTIONS.md)). Never push — the repository has no remote.
