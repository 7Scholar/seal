...
export const PLAINTEXT_LIFETIME_SECONDS = 3;

process.env.SEAL_E2E_HOME ??= mkdtempSync(join(tmpdir(), "seal-e2e-resume5-home-"));
process.env.SEAL_E2E_PICK_FOLDER ??= mkdtempSync(
  join(tmpdir(), "seal-e2e-resume5-repo-"),
);
process.env.SEAL_E2E_PLAINTEXT_LIFETIME_SECONDS ??= String(
  PLAINTEXT_LIFETIME_SECONDS,
);

export const config = {
  runner: "local",
  specs: ["./journeys/resume-after-expiry.e2e.ts"],
  maxInstances: 1,
  capabilities: [
    {
      browserName: "tauri",
