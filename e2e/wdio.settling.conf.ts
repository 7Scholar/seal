...
process.env.SEAL_E2E_HOME ??= mkdtempSync(join(tmpdir(), "seal-e2e-settling-home-"));
process.env.SEAL_E2E_PICK_FOLDER ??= mkdtempSync(
  join(tmpdir(), "seal-e2e-settling-repo-"),
);
process.env.SEAL_E2E_SECOND_REPO ??= mkdtempSync(
  join(tmpdir(), "seal-e2e-settling-second-"),
);

export const config = {
  runner: "local",
  specs: ["./journeys/settling-in.e2e.ts"],
  maxInstances: 1,
  capabilities: [
    {
      browserName: "tauri",
