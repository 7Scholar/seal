...
description: How the manage flow proposes files, why only likely secrets are pre-selected, and what the env-file editor does and does not do.
---
...
Before anything is sealed for the first time, the application asks you to acknowledge two facts it cannot soften: that a forgotten password loses the data permanently, and that sealing cannot protect a secret that was already exposed. That acknowledgement is one of only two places the application spends real friction.
...
There is no way to leave a managed file decrypted on disk. The on-disk state moves from plaintext to sealed and never back — the only action that legitimately ends with plaintext at the path is removing the file from management, which is explicit about being exactly that.
