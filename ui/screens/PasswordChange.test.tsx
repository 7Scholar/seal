...
describe("PasswordChange", () => {
  it("warns that both passwords are needed until it finishes", () => {
    setup(null);
    expect(screen.getByText(/Keep both passwords until it finishes/i)).toBeInTheDocument();
    expect(screen.getByText(/cannot be opened without it/i)).toBeInTheDocument();
  });

  it("cannot be started by reflex", () => {
    setup(null);
    expect(screen.getByRole("button", { name: /Re-encrypt every file/ })).toBeDisabled();
  });
  ...
  it("runs once everything is satisfied", async () => {
    ...
    await fill(user);
    await user.click(screen.getByRole("button", { name: /Re-encrypt every file/ }));
  ...
  it("ANSWERS WHICH FILES ARE ON WHICH PASSWORD, not a bare failure count", () => {
    ...
    expect(
      screen.getByLabelText("1 of 3 files are on the new password, 1 failed"),
    ).toBeInTheDocument();
    expect(alert).toHaveTextContent("2 still need the old one");
    expect(alert).toHaveTextContent(/Keep both until this finishes/i);
    expect(alert).toHaveTextContent(".env.production");
    expect(alert).toHaveTextContent(".env.staging");
  });
  ...
  it("surfaces an unfinished run insistently, with no way to dismiss it", () => {
    setup(partial);
    const alert = screen.getByRole("alert");
    expect(alert).toHaveClass("rekey__resume");
    expect(
      within(alert).queryByRole("button", { name: /dismiss|close/i }),
    ).toBeNull();
  });
  ...
  it("offers retrying only the rest, never redoing what converted", async () => {
    ...
    await user.type(screen.getByLabelText("New password again"), "new");
    await user.type(screen.getByLabelText(/Type/), "CHANGE MY PASSWORD");
    await user.click(screen.getByRole("button", { name: /Finish the remaining/ }));
